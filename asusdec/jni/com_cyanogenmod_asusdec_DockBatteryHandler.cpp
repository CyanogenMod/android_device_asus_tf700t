...
static void asusdec_DockBatteryHandler_nativeDockBatteryUpdate(JNIEnv* env, jobject obj)
{
    ...
    // Plugged
    setBooleanField(env, obj, gPaths.dockBatteryPluggedPath, gFieldIds.mDockBatteryPlugged);
    // Present
...
int register_asusdec_DockBatteryHandler(JNIEnv* env)
{
        ...
        while ((entry = readdir(dir))) {
            ...
            if (length > 0) {
                ...
                if(strcmp(buf, "DockBattery") == 0) {
                    ...
                    if (access(path, R_OK) == 0)
                        gPaths.dockBatteryPluggedPath = strdup(path);
                }
            }
        }
        closedir(dir);
    ...
    if (!gPaths.dockBatteryPluggedPath)
            ALOGE("dockBatteryPluggedPath not found");
    ...
    gFieldIds.mDockBatteryPlugged = env->GetFieldID(clazz, "mDockBatteryPlugged", "Z");
    ...
    LOG_FATAL_IF(gFieldIds.mDockBatteryPlugged == NULL,
            "Unable to find DockBatteryHandler.DOCK_BATTERY_PLUGGED_PATH");
